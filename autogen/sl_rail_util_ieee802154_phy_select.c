...
#define RUNTIME_PHY_SELECT (0 || SL_RAIL_UTIL_COEX_RUNTIME_PHY_SELECT)

#define RUNTIME_PHY_SELECT_STACK_SUPPORT (0 || SL_OPENTHREAD_PHY_SELECT_STACK_SUPPORT)
...
static bool checkPhySwitch(void)
{
  ...
  if ((active_radio_config != desired_radio_config)
    ...
    sl_ot_update_active_radio_config();
    return true;
  }
  return false;
}
